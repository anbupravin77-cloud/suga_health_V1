...
export default async function DoctorMessagesPage() {
  ...
  const { data } = await supabase
    ...
    .select("id, consultation_id, updated_at, consultations(primary_concern, status)")
    .eq("doctor_id", user.id)
    .order("updated_at", { ascending: false });

  return <AppShell role="doctor" active="Messages" name={profile.full_name || "Doctor"}>
    <section className="dashboard-heading">
      <div>
        <span className="eyebrow">Clinical conversations</span>
        <h1>Messages</h1>
        <p>Secure patient communication attached only to consultations assigned to you.</p>
      </div>
    </section>
        ...
        <div className="record-list spacious">
          {data.map((thread) => (
            <Link href={`/doctor/consultations/${thread.consultation_id}`} key={thread.id}>
              <div>
                <strong>{thread.consultations?.[0]?.primary_concern || "Consultation"}</strong>
                <span>Open consultation conversation</span>
              </div>
