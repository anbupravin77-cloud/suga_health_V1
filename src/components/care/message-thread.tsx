    ...
    <section className="message-panel" aria-labelledby="messages-title">
      ...
      <div className="message-list" aria-live="polite">
        {messages.length ? messages.map((message) => (
          <article className={message.sender_id === currentUserId ? "message message-own" : "message"} key={message.id}>
            <p>{message.body}</p>
      ...
      {enabled ? <form className="message-form" action={sendMessage}>
      ...
      </form> : <p className="form-note">Messaging opens when a doctor claims this consultation.</p>}
    </section>
  );
}
